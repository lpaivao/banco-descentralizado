...
class Bank:
    def __init__(self, bank_id, host_flask, port_flask):
        ...
        def escuta_existencia_conta():
            ...
            if to_account_id in self.accounts and to_bank_id is self.bank_id:
                return 'A conta existe', 200
            return 'A conta não existe', 404
                
            
    def verifica_existencia_conta(body_check_user, to_bank_id):
        check_user = requests.post(f'localhost:800{to_bank_id}/transferencia/existencia_conta', body_check_user)
